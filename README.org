...
Surely now you must be asking yourself "But why can't you handle the end of line
character my dear system?" The answer is simple: it makes it easy for you create
initialism starting from wherever you like, skipping or whatever you wish, there
are no limits on the source of your initialism. Therefore I assume nothing
beyond the fact that initialism are built using only the [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Char-Classes.html#Char-Classes][Character Classes]]
=[[:alnum:]]= and =[[:punct:]]=
